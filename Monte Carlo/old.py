...
class Integral:
    ...
    def set_arguments(self):
        self.arguments = create_array(self.x1, self.x2, self.dots)

    def set_random_values(self):
        self.random_values = create_array(self.y1, self.y2, self.dots)
    ...
    def calculate(self):
        ...
        # plt.show()  # takes time
        return (self.x2 - self.x1) * (self.y2 - self.y1) * (m / self.dots)
...
def main():
    a, b = - 1 / 2, 1 / 2
    A, B = 0, np.pi
    integral = Integral(a, b, A, B, 10000)
    print(integral.calculate())
